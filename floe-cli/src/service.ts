 ...
 *
 * Floe's start policy (services.start_on_demand) decides whether a client may
 * start the substrate on demand. That is a desktop convenience and is
 * deliberately wrong for a managed deployment, where the substrate is owned by
 * the machine and a client quietly starting its own copy is a correctness
 * problem. So the honest answer, when a person keeps hitting "not running", is
 * to have the machine start Floe for them — a real platform auto-start mechanism.
 *
