...
it("freezes legacy visible attachment history once with explicit provenance and a recovery copy", async () => {
  const root = mkdtempSync(join(tmpdir(), "floe-event-history-"));
  const configPath = join(root, "config.yaml");
  const config = defaultConfig(root);
  writeFileSync(configPath, YAML.stringify(config), "utf8");
  let handle = await createBusServer(configPath, config, { unsafe_in_process_test_auth_bypass: true });
  await handle.app.ready();
  let store: BusStore = handle.store;
  try {
    ...
    const current = (await emitViaRoute(handle, {
      type: "message", workspace_id: workspace, source_endpoint_id: actor,
